...
def terra_search(query):
    ...
    db_data = database.fetch_values_from_table_for_genral_terms(queries)
    # print(db_data.count())
    ...
    # when value is not [resent in database will call the search function to fetch  the data from google api
        ...
        res_df = search(query)
        res_df.apply(lambda row: database.insert_into_table(row),axis=1)
        return res_df
